...
export default async function MyChampionships() {
  ...
  return (
    <main className="min-h-screen ml-0 sm:ml-25 lg:ml-68 bg-neutral-150 p-4">
      ...
      <section className="flex flex-col gap-8 w-full max-w-4xl mx-auto">
        {leagues.length > 0 ? (
          leagues.map((league) => (
        ...
        <Link href="/create-championship">
          <button className="bg-zinc-950 text-white h-32 w-full rounded-md cursor-pointer hover:bg-zinc-800 transition-colors font-medium">
            + Criar Torneio
          </button>
        </Link>
      </section>
